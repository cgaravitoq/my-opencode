...
| `status:hitl`   | All tasks done, PR open, awaiting merge or human review depending on the PR label.          | `prd-to-execution` → human / automation |
...
- `prd-to-execution/SKILL.md`: execute one `status:prd` issue inside its target repo through a single parent branch and one PR.
...
- One executable PRD = one parent branch + one PR + one GitHub issue. Task coordination during execution lives inline in the parent body (markdown checkboxes plus `Status` and `Commit` fields) and issue comments. No sub-issues are created for tasks.
...
- The agent moves `status:prd → status:running` only when the first execution task starts. It moves `status:running → status:hitl` only when every task is done, the PR exists, verification passed, and the PR label is set. It does not move issues to `status:ready`; the human (or a merge) owns that.
...
4. Use `prd-to-execution` to execute exactly one `status:prd` issue at a time. Resolve the repo, switch to it, run the parent branch + PR loop, and hand off either an `approved` PR for merge or a `hitl` PR for human review.
...
4. `prd-to-execution` resolved the target repo, recorded it on the issue, ran every execution-plan task inline (body checkbox + `Status` + `Commit`) on the parent issue, started one shared branch, recorded implementation evidence in the body and issue comments, and shipped one PR.
5. Final handoff to the user includes: issue state, repo, branch, commit, PR URL, PR label, verification evidence, and any non-goals that were skipped.
