...
description: Execute a `status:prd` GitHub issue inside its target repo, tracking task progress inline (body checkboxes + issue comments) on the parent issue, committing all task work into a single parent branch, and finishing in one shared draft PR. Resolves the target repo from the issue body. Use when the user asks to execute a GitHub PRD or connect issue work to a branch/PR.
---
...
End-to-end: resolve the target repo, switch into it, prepare the parent spec, then **delegate code work to the global `pipeline-execution` skill** (which owns `exec → reviewer → fixer × ≤3 → draft PR`). This skill only handles GitHub Issues bookkeeping — label transitions, body checkboxes, issue comments, `## Implementation` updates.
...
- The parent spec is the only delivery unit and the only GitHub issue: one parent branch, one draft PR, one set of inline checkboxes, one comment timeline.
...
- Record branch, commit, draft PR URL, verification output, and changed-file summary on the parent issue before moving it to `status:hitl`.
...
- Move the parent to `status:hitl` only after every task checkbox is `[x]`, the parent branch + commit hash + draft PR URL + verification output are recorded in the body, and the comment timeline includes a completion comment for each task. Single edit: `gh issue edit <N> --remove-label status:running --add-label status:hitl`.
...
Code work is **not** implemented inline in this skill. Delegate the entire `exec → reviewer → fixer × ≤3 → draft PR` loop to the global `pipeline-execution` skill. This bundle only handles GitHub Issues bookkeeping around it.
...
5. **PRD-level run** (after all per-task work is committed): invoke `pipeline-execution` once more with the full task list collapsed to a single "post-integration" entry, the PRD `## Verify` block as `verify_command`, the GitHub issue URL as `tracker_url`, the PR title and summary built from `## What`, and `pr_label_clean: hitl` / `pr_label_blocked: hitl-blocked`. This run owns the swarm, the fixer loop, the final verify gate, and the draft PR push. The reviewer ensures the PR body contains `Closes #<N>` (or `Closes owner/repo#<N>` cross-repo) so the parent issue auto-closes on merge.
...
6. **After the final `pipeline-execution` returns**: update `## Implementation` with the final branch, latest commit hash (after fixer passes if any), draft PR URL, label applied (`hitl` or `hitl-blocked`), the review loop summary (passes used, blockers resolved, residual concerns), and the final verify evidence.
...
If the reviewer (inside `pipeline-execution`) returns `hitl-blocked`, still move the parent to `status:hitl`. The PR is open with the `hitl-blocked` label and residual concerns in its body — the human owns the next step. Surface this explicitly to the user; do not silently retry the loop.
...
- **Returns `hitl-blocked`** → move the parent to `status:hitl`, record `Blocked: <count> concerns — see PR body` under `## Implementation`. Surface explicitly to the user.
...
- shared branch, latest commit hash, draft PR URL, label applied (`hitl` or `hitl-blocked`)
...
> *"HITL handoff: review loop closed clean. Review the draft PR `<pr-url>`, then run `gh pr ready <pr-number>` to flip it out of draft. The PR body has `Closes <owner/repo#N>` so the issue label flips to `status:ready` automatically on merge."*

For `hitl-blocked` (loop exhausted):

> *"HITL handoff with unresolved blockers: the review-fix loop hit 3 passes without converging. Open the draft PR `<pr-url>` (label: `hitl-blocked`) — its body lists the residual concerns. You can: (1) resolve them manually and run `gh pr ready`, (2) re-run the architect with a tighter scope, or (3) merge as-is if the concerns are acceptable. The issue flips to `status:ready` automatically on merge via the PR's `Closes` reference."*
