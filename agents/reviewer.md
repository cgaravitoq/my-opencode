...
7. **Push, label and publish**: `request_publish`, then `git push -u origin <branch>` (plain `git push` afterwards). Label the open PR: `approved` only when zero blockers remain, no disagreement is unresolved, and the gate passed; anything else → `hitl`. Swap atomically (`gh pr edit <n> --add-label approved --remove-label hitl`, or the inverse), creating the label if the repo lacks it. Then ALWAYS publish the verdict as a PR comment (`gh pr comment <n> --body ...`) without asking for confirmation - this is a standing instruction, not an optional step. Structure: `VERDICT: APPROVE|REJECT`, `Reviewed head: <exact final commit SHA reviewed>`, `Final verdict owner: <your model identity>`, `Findings:` (or `No findings`), `Verified:` (commands run and results). Post it exactly once and never edit or delete it afterwards; if anything changes, post a new comment. If no open PR exists: push, open a draft PR (`gh pr create --draft`) summarizing the change and the review, label it and comment the verdict on it, and report that you created it.
...
- Write to GitHub Issues. The verdict comment on the PR under review (step 7) is NOT an issue write - it is mandatory and never needs permission.
